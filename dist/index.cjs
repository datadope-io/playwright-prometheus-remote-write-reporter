...
__export(index_exports, {
  Counter: () => Counter,
  Gauge: () => Gauge,
  default: () => PrometheusReporter
});
module.exports = __toCommonJS(index_exports);
var import_node_path = __toESM(require("path"), 1);
var import_node_process = require("process");
var import_node_os = require("os");
var import_prometheus_remote_write = require("prometheus-remote-write");
...
var PrometheusReporter = class {
  ...
  node_argv = new Counter(
    {
      name: "node_argv",
      ...Object.fromEntries(
        import_node_process.argv.map((value, index) => [index, value])
      )
    },
    1
  );
  node_os = new Counter(
    {
      ...
      cpusCount: String((0, import_node_os.cpus)().length),
      availableParallelism: String((0, import_node_os.availableParallelism)()),
      machine: (0, import_node_os.machine)(),
      username: (0, import_node_os.userInfo)().username,
      shell: (0, import_node_os.userInfo)().shell ?? "",
      uid: String((0, import_node_os.userInfo)().uid),
      gid: String((0, import_node_os.userInfo)().gid),
      homedir: (0, import_node_os.userInfo)().homedir,
      platform: (0, import_node_os.platform)(),
      release: (0, import_node_os.release)(),
      type: (0, import_node_os.type)(),
      version: (0, import_node_os.version)()
    },
    1
  );
  node_env;
  ...
  cpuDelta;
  updateNodejsStats() {
    this.cpuDelta = (0, import_node_process.cpuUsage)(this.cpuDelta);
    this.node_cpu_user.set(this.cpuDelta.user);
    this.node_cpu_system.set(this.cpuDelta.system);
    this.node_memory_free.set((0, import_node_os.freemem)());
    this.memoryDelta = (0, import_node_process.memoryUsage)();
  ...
  async send(series) {
    await (0, import_prometheus_remote_write.pushTimeseries)(series, this.options);
  }
  ...
  async onTestEnd(test, result) {
    ...
    for (const step of result.steps) {
      ...
      const location = step.location ? `${import_node_path.default.relative(process.cwd(), step.location.file)}:${step.location.line}:${step.location.column}` : "unknown";
      const raw_id = `${step.title}:${step.category}:${location}`;
      const stepId = import_node_crypto.default.createHash("md5").update(raw_id).digest("hex");
      const labels2 = {
        stepId,
        testId: test.id,
        testTitle: test.title,
  ...
  location(test) {
    const relativePath = import_node_path.default.relative(process.cwd(), test.location.file);
